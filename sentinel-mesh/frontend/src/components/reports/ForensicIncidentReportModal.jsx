...
export default function ForensicIncidentReportModal({
  isOpen,
  onClose,
  incident,
  events = [],
  ...
  // Connected real NSL-KDD evidence packets (with actively inspected packet from screen highlighted at top)
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/85 backdrop-blur-md p-4 overflow-y-auto font-mono">
      <div className="relative w-full max-w-4xl bg-[#060a14] border-2 border-cyan-700/80 shadow-[0_0_50px_rgba(0,240,255,0.2)] text-slate-100 my-8">
        {/* Top Header Bar */}
        <div className="bg-[#080e20] border-b border-cyan-800/80 p-4 sm:p-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-rose-950 border border-rose-500 flex items-center justify-center text-rose-400">
              <ShieldAlert className="w-6 h-6 animate-pulse" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs text-rose-400 font-bold uppercase tracking-wider">
                  CLASSIFIED // FORENSIC INCIDENT REPORT
                </span>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-[#0b1329] border border-cyan-800 text-cyan-300 text-xs font-bold hover:bg-cyan-950 transition-colors cursor-pointer"
            >
        ...
        <div className="p-5 sm:p-7 space-y-6 max-h-[75vh] overflow-y-auto">
          {/* Metadata Grid */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-[#080d1a] border border-slate-800 p-3 text-xs">
            <div>
            ...
            <div>
              ...
              <span className="font-bold text-cyan-300 text-sm mt-0.5 block">{Math.round((inc.confidence || 0.94) * 100)}% VERIFIED</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className="font-bold text-amber-300 text-xs mt-1 uppercase block">{inc.status || 'NEW'}</span>
            </div>
          </div>
          ...
          <div className="border border-slate-800 bg-[#070b16] p-4">
            <div className="flex items-center justify-between pb-2 mb-2 border-b border-slate-800 text-xs font-bold text-slate-300">
              <span className="uppercase text-cyan-400">Executive Forensic Narrative</span>
          ...
          {/* Section 1: Digital Packet Telemetry (Real NSL-KDD Evidence) */}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse font-mono">
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {kddEvidence.map((pkt) => (
                    <tr key={pkt.packet_id} className="hover:bg-slate-900/50">
                      <td className="py-1.5 px-2.5 text-cyan-300 font-bold">{pkt.packet_id}</td>
          ...
          {/* Section 2: Physical & Optical Evidence */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="border border-slate-800 bg-[#070b16] p-4 space-y-2">
              <div className="flex items-center gap-2 text-xs font-bold text-emerald-400 uppercase pb-2 border-b border-slate-800">
                ...
                <span>2. Physical Access Telemetry</span>
              </div>
          ...
          {/* Section 3: Recommended Containment Plays */}
        ...
        <div className="bg-[#080d1a] border-t border-slate-800 p-4 flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="text-slate-500">
            Chain of Custody Hash: <span className="text-cyan-400">SHA256:7f8a91c0e8f0a2e4...88b2</span>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 bg-cyan-950 hover:bg-cyan-900 border border-cyan-600 text-cyan-200 font-bold cursor-pointer"
          >
            DISMISS REPORT
          </button>
        </div>
