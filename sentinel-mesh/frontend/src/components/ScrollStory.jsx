...
export default function ScrollStory() {
  ...
  // Ultra-Smooth GSAP Horizontal Scroll Setup
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tweenRef.current = gsap.to(panelsRowRef.current, {
        x: () => -totalWidth,
        ease: 'none',
        scrollTrigger: {
          ...
          start: 'top top',
          end: () => `+=${(TOTAL_PANELS - 1) * window.innerHeight * 1.3}`, // Generous scroll distance for ultra-smooth scrub
  ...
  return (
    ...
    >
      {/* Precision Tactical Cursor & Particle Click Sparks */}
      <TargetCursor />
      <ClickSpark />
      ...
      >
        ...
        {/* Sub-Header Horizontal Panel Track Buttons */}
          ...
          <div className="flex items-center gap-2 overflow-x-auto scrollbar-none">
            {panelNames.map((name, idx) => (
              <button
                ...
                className={`px-3 py-1 border transition-all cursor-pointer whitespace-nowrap ${
                  activePanel === idx
                    ? 'border-cyan-400 bg-cyan-950/60 text-cyan-300 font-bold shadow-[0_0_10px_rgba(0,240,255,0.2)]'
                    : 'border-slate-800 bg-[#070b14] text-slate-500 hover:text-slate-300'
                }`}
              >
                {name}
              </button>
      ...
      {/* Legal & Compliance Modals (T&C and Privacy Policy) */}
      <LegalModals
